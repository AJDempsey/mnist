...
def main(unused_argv):
    ...
    # Create Estimator (a TensorFlow class for performing high-level model training, evaluation, and
    # inference in depth walk through at https://www.tensorflow.org/get_started/custom_estimators )
    # Pass the model we've built and a directory for model data (checkpoints)
    mnist_classifier = tf.estimator.Estimator(model_fn=cnn_model_fn, model_dir='C:\\temp\\checkpoint')

    # Set up logging for predictions, this will show us that the CNN is still running.
    # The Logging will give us the probabilities from the softmax layer of the CNN
    # Key is the value to log out, the value is name of the Tensor in the graph (line 80ish)
